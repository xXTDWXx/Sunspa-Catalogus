(function(){
  ...
  const searchable = (prod) => {
    const parts = [
      prod.id, prod.type, prod.title, prod.url,
      ...(prod.bullets || []),
      ...(prod.specs || []).flatMap(s => [s.label, s.value])
    ].map(safeText).join(" ").toLowerCase();
    return parts;
  };

  const SEARCH_INDEX = new Map(PRODUCTS.map(p => [p.id, searchable(p)]));
